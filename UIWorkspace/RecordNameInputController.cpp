...
void RecordNameInputController::Start()
{
	...
	auto br = m_indicator->AddComponent<BitmapRenderer>(L"Image/LetterIndicator.bmp");
	br->SetSpaceType(SpaceType::Screen);
	br->SetSortOrder(1);

	m_indicator->GetTransform()->SetLocalPosition(
		GetTransform()->GetLocalPosition() + m_letterPositions[0]);
}
...
void RecordNameInputController::Update()
{
	if (m_screenCurtainController == nullptr)
	{
		if (Input::IsKeyPressed(VK_LEFT))
		{
			...
			m_indicator->GetTransform()->SetLocalPosition(
				GetTransform()->GetLocalPosition() + m_letterPositions[m_letterIndicatorIndex]);
		...
		if (Input::IsKeyPressed(VK_UP))
		{
			...
			if (index == CharacterSpriteIndex::Num0)
			{
		...
		if (Input::IsKeyPressed(VK_DOWN))
		{
			...
			if (index == CharacterSpriteIndex::AlphaZ)
			{
		...
		if (Input::IsKeyHeld(VK_UP))
		{
			if (m_isHoldingUp)
			{
				...
				if (m_scrollStartTimer >= m_scrollDelay)
				{
					...
					{
						m_scrollIndex = 35.99f;
					}

					CharacterSpriteIndex index = static_cast<CharacterSpriteIndex>(m_scrollIndex);
