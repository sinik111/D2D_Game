...
PlayerHeavyAttackTwiceState::PlayerHeavyAttackTwiceState(Player* player)
	: PlayerStateBase(player)
{
}

void PlayerHeavyAttackTwiceState::Enter(FSMContext& context)
{
	m_eventDone = false;
	...
	m_attackAnimationDuration = m_player->m_attackHeavyTwiceDuration;

	m_player->m_lastAttackName = L"HeavyAttackTwice";

	//m_player->PlayerHeavyAttackTwice(m_attackAnimationDuration);

	m_player->PlayerNormalAttackAll(m_attackAnimationDuration);
}

void PlayerHeavyAttackTwiceState::Update(FSMContext& context)
{
...
void PlayerHeavyAttackTwiceState::Exit(FSMContext& context)
{
	m_player->m_onAttack = false;
	m_player->GetPlayerStatus().attackIntervalTimer = 0.0f;
}

void PlayerHeavyAttackTwiceState::CheckCancel(FSMContext& context)
{
