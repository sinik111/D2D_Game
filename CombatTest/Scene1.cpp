...
void Scene1::Enter()
{
	...
	go->GetTransform()->SetLocalPosition(-100.0f, 100.0f);
	go->GetComponent<EnemyBase>()->SetTargetPlayerAndCamera(player, camera);

	//go = CreateGameObject(L"EnemyBase");
	//br1 = go->AddComponent<BitmapRenderer>();
	//br1->SetSortOrder(1);
	//go->AddComponent<Animator>();
	//go->AddComponent<PlayerInput>();
	//go->AddComponent<TextRenderer>();
	...
	//go->AddComponent<RigidBody2D>();
	//go->AddComponent<BoxCollider2D>();
	//go->GetTransform()->SetLocalPosition(500.0f, -300.0f);
	//go->GetComponent<EnemyBase>()->SetTargetPlayerAndCamera(player, camera);
}
