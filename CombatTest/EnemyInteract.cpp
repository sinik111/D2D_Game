...
void EnemyInteract::HitEnemy( const Player* pPlayer, const std::wstring& objName)
{
	...
	if (objName == L"PlayerNormalAttack")
	{
		knockdownPow = pPlayer->GetPlayerStat().attackKnockdownPower;

		dmg = Random::Int(pPlayer->GetPlayerStat().attackPowerMin, pPlayer->GetPlayerStat().attackPowerMax);
	}
	else if (objName == L"PlayerHeavyAttack")
	{
		knockdownPow = pPlayer->GetPlayerStat().heavyAttackKnockdownPower;

		dmg = Random::Int(pPlayer->GetPlayerStat().heavyAttackPowerMin, pPlayer->GetPlayerStat().heavyAttackPowerMax);
	}	
