...
void PlayerNormalAttackState::Enter(FSMContext& context)
{
	...
	m_attackAnimationDuration = m_player->m_attackNormalDuration;
	
	m_player->m_lastAttackName = L"NormalAttack";

	m_player->PlayerNormalAttack1(m_attackAnimationDuration);
}
...
void PlayerNormalAttackState::Update(FSMContext& context)
{
	...
	if (m_attackAnimationTimer >= m_attackAnimationDuration)
	{
		...
		{
			Debug::Log("공격스테이트에서 다음으로 진행");
			m_player->m_isComboWait = false;
			m_player->m_isNextAttackQueued = false; // 플래그 초기화
			m_player->ChangeStateAndAnim(L"NormalAttackTwice"); // 다음 콤보로 전환
		}
		else
		{			
...
void PlayerNormalAttackState::CheckCancel(FSMContext& context)
{
