...
void EnemyElite::Initialize()
{
	OriginPos() = GetGameObject()->GetTransform()->GetLocalPosition();
	OriginAngle() = -90.0f;
	LoadJson(L"StatData/EnemyEliteStat.json");
}


void EnemyElite::OnCollisionEnter(const Collision& collision)
{
	Debug::Log(L"EnemyElite OnCollisionEnter");
}

void EnemyElite::OnCollisionStay(const Collision& collision)
{
}

void EnemyElite::OnCollisionExit(const Collision& collision)
{
}

void EnemyElite::OnTriggerEnter(const Collision& collision)
{
	Debug::Log(L"EnemyElite OnTriggerEnter");
}

void EnemyElite::OnTriggerExit(const Collision& collision)
{
	Debug::Log(L"EnemyElite OnTriggerExit");
}



void EnemyElite::AnimationInit()
{
	InitAnimPath(m_idleAnimPath, L"EliteEnemyIdle");
	InitAnimPath(m_attack1_1AnimPath, L"EliteEnemyHorizontalAttack001");
	InitAnimPath(m_attack1_2AnimPath, L"EliteEnemyHorizontalAttack002");
	InitAnimPath(m_attack2_1AnimPath, L"EliteEnemyStabAttack");
	InitAnimPath(m_KnockdownAnimPath, L"EliteEnemyKnockDown");
	InitAnimPath(m_moveAnimPath, L"EliteEnemyMove");
	InitAnimPath(m_dieAnimPath, L"EliteEnemyDie");

	m_animator->AddActionOnEvent(L"Eite_Enemy_StabAttack", this, &EnemyElite::EnemyStabAttack);
	m_animator->AddActionOnEvent(L"Enemy_Atteck001", this, &EnemyElite::EnemyAttack);

	SetAnimState(EnemyAnimIdle);
}


void EnemyElite::CheckAndTransitAnimState()
{
...
void EnemyElite::EnemyStabAttack()
{
	Vector2 position = GetTransform()->GetLocalPosition();
	SoundManager::Get().PlaySound(L"EnemyStabAttack", SoundType::SFX, &position);
}

void EnemyElite::EnemyAttack()
{
	Vector2 position = GetTransform()->GetLocalPosition();
	SoundManager::Get().PlaySound(L"EnemyAttack", SoundType::SFX, &position);
}

void EnemyElite::ColliderInit()
{
...
void EnemyElite::InitAnimPath(AnimPathArray& animPath, const std::wstring& animName)
{
	if (animName == L"EliteEnemyHorizontalAttack001")
	{
		
		std::wstring spriteSheetPath = L"EliteEnemy\\EliteEnemyHorizontalAttack001\\EliteEnemyHorizontalAttack001_sprites.json";
		...
		for (size_t i = 0; i < animPath.size(); ++i)
		{
			...
			std::wstring animationClipPath = L"EliteEnemy\\" + animName + L"\\" + baseFileName + L"_anim.json";
			m_animator->AddAnimationClip(animationClipPath);
		}
	}
	else if (animName == L"EliteEnemyHorizontalAttack002")
	{

		std::wstring spriteSheetPath = L"EliteEnemy\\EliteEnemyHorizontalAttack002\\EliteEnemyHorizontalAttack002_sprites.json";
		m_animator->AddSpriteSheet(spriteSheetPath);
	...
	else
	{
		for (size_t i = 0; i < animPath.size(); ++i)
		{
			...
			std::wstring spriteSheetPath = L"EliteEnemy\\" + animName + L"\\" + baseFileName + L"_sprites.json";
			m_animator->AddSpriteSheet(spriteSheetPath);
...
void EnemyElite::DebugTextUpdate()
{
...
void EnemyElite::CheckAndTransitState()
{
...
void EnemyElite::SetAnimState(int animState)
{
	AnimPathArray temp;
...
void EnemyElite::Start()
{
	...
	SoundManager::Get().PlaySound(L"EnemyIdle", SoundType::SFX, &position, name, true);
...
void EnemyElite::Update()
{
	Vector2 position = GetTransform()->GetLocalPosition();
	std::wstring name = L"Enemy" + std::to_wstring(m_uniqueId);
	SoundManager::Get().Update3DAttribute(name, position);
