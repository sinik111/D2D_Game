...
void RankingViewer::LoadRankingData()
{
	...
	for (int i = previousRows; i < previousRows + m_currentRows; ++i)
	{
		...
		m_batchRenderer->AddBatchUnit(
			{
				static_cast<size_t>(CharacterSpriteIndex::LeftArrow),
				Vector2(-200.0f, m_pageViewPositionY),
				Vector2(1.0f, 1.0f),
				0.0f,
				{ 1.0f, 1.0f, 1.0f, 1.0f }
			}
		...
		count = 0;
		auto pageNumIndexList = ConvertPageNumbersRightToLeft(m_currentPage + 1);
		for (auto index : pageNumIndexList)
		{
			m_batchRenderer->AddBatchUnit(
				{
					static_cast<size_t>(index),
					Vector2(-40.0f + -45.0f * count, m_pageViewPositionY),
					Vector2(1.0f, 1.0f),
					0.0f,
					{ 1.0f, 1.0f, 1.0f, 1.0f }
				}
		...
		m_batchRenderer->AddBatchUnit(
			{
				static_cast<size_t>(CharacterSpriteIndex::Slash),
				Vector2(0.0f, m_pageViewPositionY),
				Vector2(1.0f, 1.0f),
				0.0f,
				{ 1.0f, 1.0f, 1.0f, 1.0f }
			}
		...
		count = 0;
		auto totalPageNumIndexList = ConvertPageNumbersLeftToRight(m_totalPages);
		for (auto index : totalPageNumIndexList)
		{
			m_batchRenderer->AddBatchUnit(
				{
					static_cast<size_t>(index),
		...
		m_batchRenderer->AddBatchUnit(
			{
				static_cast<size_t>(CharacterSpriteIndex::RightArrow),
				Vector2(200.0f, m_pageViewPositionY),
				Vector2(1.0f, 1.0f),
				0.0f,
				{ 1.0f, 1.0f, 1.0f, 1.0f }
